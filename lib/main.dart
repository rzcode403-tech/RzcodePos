...
class Sale {
  ...
  factory Sale.fromMap(Map<String, dynamic> m) {
    ...
    return Sale(
      ...
      date:     DateTime.tryParse((m['sale_date'] as String?) ?? '') ?? DateTime.now(),
      ...
      given:    ((m['given']    as num?)  ?? 0).toDouble(),
      change:   ((m['change_am'] as num?) ?? 0).toDouble(),
    );
  }
}
...
class AppLog {
  ...
  factory AppLog.fromMap(Map<String, dynamic> m) => AppLog(
    id:      (m['id']       as int?)    ?? 0,
    userId:  (m['user_id']  as int?)    ?? 0,
    date:    DateTime.tryParse((m['log_date'] as String?) ?? '') ?? DateTime.now(),
...
class DB {
  ...
  static Future<Database> _init() async {
    ...
    final path = p.join(dir, 'caisse_pos.db');
  ...
  static Future<void> _create(Database db, int v) async {
    ...
    await db.execute('''CREATE TABLE categories(
      ...
      emoji TEXT DEFAULT "🏷️", color TEXT DEFAULT "#1b3a5c", status INTEGER DEFAULT 1)''');
    await db.execute('''CREATE TABLE products(
      id INTEGER PRIMARY KEY, name TEXT NOT NULL, cat INTEGER DEFAULT 1,
      price REAL DEFAULT 0, tva INTEGER DEFAULT 0, stock INTEGER DEFAULT 0,
      emoji TEXT DEFAULT "📦", barcode TEXT DEFAULT "", status INTEGER DEFAULT 1)''');
    await db.execute('''CREATE TABLE users(
      id INTEGER PRIMARY KEY, username TEXT UNIQUE NOT NULL, password TEXT NOT NULL,
      prenom TEXT DEFAULT "", nom TEXT DEFAULT "", tel TEXT DEFAULT "",
      email TEXT DEFAULT "", role TEXT DEFAULT "Vendeur",
      status INTEGER DEFAULT 1, last_login TEXT)''');
    await db.execute('''CREATE TABLE sales(
      id INTEGER PRIMARY KEY, sale_date TEXT NOT NULL, cashier TEXT DEFAULT "",
      user_id INTEGER DEFAULT 0, items TEXT DEFAULT "[]",
      subtotal REAL DEFAULT 0, tax REAL DEFAULT 0, total REAL DEFAULT 0,
      method TEXT DEFAULT "Espèces", given REAL DEFAULT 0, change_am REAL DEFAULT 0)''');
    await db.execute('''CREATE TABLE logs(
      id INTEGER PRIMARY KEY, log_date TEXT NOT NULL, user_id INTEGER DEFAULT 0,
      username TEXT DEFAULT "", prenom TEXT DEFAULT "", role TEXT DEFAULT "",
  ...
  static Future<void> _seed(Database db) async {
    // Settings
    const st = {
      'name':'SuperMarché Rzouga','slogan':'Qualité & Fraîcheur',
      'addr':'Avenue Habib Bourguiba','city':'Tunis',
      'tel':'+216 71 000 000','email':'contact@supermarche.tn',
      'mf':'1234567/A/M/000','rne':'J0123456','tva':'19',
      'msg':'Merci de votre visite !','logo':'🛒','cur':'DT',
    };
    ...
    final cats = [
      [1,'Épicerie','🛒','#1b3a5c'],
      [2,'Fruits & Légumes','🥦','#16a34a'],
      [3,'Boucherie','🥩','#dc2626'],
      [4,'Boulangerie','🥖','#d97706'],
      [5,'Produits Laitiers','🥛','#2563eb'],
      [6,'Boissons','🧃','#7c3aed'],
      [7,'Hygiène','🧴','#0891b2'],
    ];
    for (final c in cats) {
      await db.insert('categories',
        {'id':c[0],'name':c[1],'emoji':c[2],'color':c[3],'status':1});
    }
    // Products
    final prods = [
      [1,'Baguette',4,0.300,0,200,'🥖','3017620429484'],
      [2,'Lait Entier 1L',5,1.500,7,150,'🥛','6191234000001'],
      [3,'Tomates 1kg',2,1.200,0,80,'🍅','6191234000002'],
      [4,'Pommes Golden 1kg',2,2.000,0,60,'🍎','6191234000003'],
      [5,'Yaourt Nature x4',5,2.400,7,90,'🍶','6191234000004'],
      [6,'Eau Delice 1.5L',6,0.450,0,300,'💧','6191494000026'],
      [7,'Jus Orange 1L',6,2.800,19,70,'🧃','6191234000006'],
      [8,'Coca-Cola 50cl',6,1.800,19,120,'🥤','5449000000996'],
      [9,'Poulet Entier',3,8.500,7,30,'🍗','6191234000008'],
      [10,'Viande Hachée 500g',3,12.000,7,25,'🥩','6191234000009'],
      [11,'Huile Végétale 1L',1,5.500,7,50,'🫙','6191234000010'],
      [12,'Sucre 1kg',1,2.200,7,100,'🍬','6191234000011'],
      [13,'Farine 1kg',1,1.800,7,80,'🌾','6191234000012'],
      [14,'Fromage Kiri x8',5,4.200,7,45,'🧀','7613035349094'],
      [15,'Shampooing 250ml',7,6.900,19,35,'🧴','6191234000015'],
      [16,'Savon x6',7,3.500,19,60,'🧼','6191234000016'],
      [17,'Pâtes 500g',1,1.400,7,90,'🍝','6191234000017'],
      [18,'Riz 1kg',1,2.600,7,75,'🍚','6191234000018'],
      [19,'Bananes 1kg',2,1.800,0,55,'🍌','6191234000019'],
      [20,'Café Moulu 250g',1,7.500,7,40,'☕','6191234000020'],
    ];
    for (final r in prods) {
      await db.insert('products', {
        'id':r[0],'name':r[1],'cat':r[2],'price':r[3],
        'tva':r[4],'stock':r[5],'emoji':r[6],'barcode':r[7],'status':1,
      });
    }
    // Users
    final users = [
      [1,'admin','admin123','Abderrazek','Ouertani','+216 92 862 052','admin@rzcode.tn','Admin'],
      [2,'superviseur','sup123','Fatima','Trabelsi','+216 71 222 222','fatima@supermarche.tn','Superviseur'],
      [3,'vendeur','vend123','Mohamed','Khelifi','+216 71 333 333','med@supermarche.tn','Vendeur'],
    ];
    for (final u in users) {
      await db.insert('users', {
        'id':u[0],'username':u[1],'password':u[2],'prenom':u[3],
        'nom':u[4],'tel':u[5],'email':u[6],'role':u[7],'status':1,
      });
    }
  }
  ...
  static Future<List<Category>> getCategories() async {
    final db = await database;
    return (await db.query('categories', orderBy: 'id')).map(Category.fromMap).toList();
  }
  static Future<void> saveCategory(Category c) async {
    final db = await database;
  ...
  static Future<List<Product>> getProducts() async {
    final db = await database;
    return (await db.query('products', orderBy: 'id')).map(Product.fromMap).toList();
  }
  ...
  static Future<Product?> getByBarcode(String code) async {
    ...
    final r  = await db.query('products',
      where: 'barcode=? AND status=1', whereArgs: [code], limit: 1);
  ...
  static Future<List<AppUser>> getUsers() async {
    final db = await database;
    return (await db.query('users', orderBy: 'id')).map(AppUser.fromMap).toList();
  }
  static Future<void> saveUser(AppUser u) async {
    final db = await database;
  ...
  static Future<AppUser?> login(String un, String pw) async {
    ...
    final r  = await db.query('users',
      where: 'username=? AND password=? AND status=1', whereArgs: [un, pw], limit: 1);
    if (r.isEmpty) return null;
    final u = AppUser.fromMap(r.first);
  ...
  static Future<List<Sale>> getSales() async {
    final db = await database;
    return (await db.query('sales', orderBy: 'sale_date DESC', limit: 500))
      .map(Sale.fromMap).toList();
  }
  static Future<void> saveSale(Sale s) async {
    final db = await database;
    await db.insert('sales', s.toMap(), conflictAlgorithm: ConflictAlgorithm.ignore);
  }
  ...
  static Future<List<AppLog>> getLogs() async {
    final db = await database;
    return (await db.query('logs', orderBy: 'log_date DESC', limit: 500))
      .map(AppLog.fromMap).toList();
  }
  static Future<void> addLog(AppLog l) async {
    final db = await database;
    await db.insert('logs', l.toMap(), conflictAlgorithm: ConflictAlgorithm.ignore);
  }
...
class AppState extends ChangeNotifier {
  ...
  static int newId() {
    _ctr = (_ctr + 1) % 9999;
    return DateTime.now().millisecondsSinceEpoch * 10000 + _ctr;
  }
  ...
  Future<bool> doLogin(String un, String pw) async {
    final u = await DB.login(un, pw);
    if (u == null) {
      _addLog('LOGIN_FAIL', '', 'Tentative échouée : $un', 0, '—', '—');
      return false;
    }
    currentUser = u;
    _addLog('LOGIN', u.prenom, '${u.prenom} ${u.nom} connecté', u.id, u.username, u.role);
    notifyListeners();
    return true;
  }
  ...
  void doLogout() {
    if (currentUser != null) {
      _addLog('LOGOUT', currentUser!.prenom,
        '${currentUser!.prenom} ${currentUser!.nom} déconnecté',
  ...
  Future<Sale> processSale(String method, double given) async {
    final u = currentUser!;
    final items = cart.map((i) => SaleItem(
      productId: i.product.id, name: i.product.name, emoji: i.product.emoji,
      price: i.product.price, tva: i.product.tva, qty: i.qty)).toList();

    final sale = Sale(
      id: newId(), date: DateTime.now(), cashier: u.username,
      userId: u.id, items: items, subtotal: subTotal, tax: taxTotal,
      total: grandTotal, method: method, given: given,
      change: method == 'Espèces' ? (given - grandTotal).clamp(0, 1e9) : 0,
    );
    ...
    for (final ci in cart) {
      ...
      if (idx >= 0) {
        ...
        await DB.updateStock(products[idx].id, products[idx].stock);
      }
    }

    await DB.saveSale(sale);
    sales.insert(0, sale);
    _addLog('SALE', u.prenom, 'Vente — ${fmtP(sale.total)} — $method',
  ...
  Future<void> saveProduct(Product pr) async {
    await DB.saveProduct(pr);
    final i = products.indexWhere((p) => p.id == pr.id);
    if (i >= 0) products[i] = pr; else products.add(pr);
    notifyListeners();
  }
  ...
  Future<void> saveCategory(Category c) async {
    ...
    if (i >= 0) categories[i] = c; else categories.add(c);
    notifyListeners();
  }
  ...
  Future<void> saveUser(AppUser u) async {
    ...
    if (i >= 0) users[i] = u; else users.add(u);
    notifyListeners();
  }
  ...
  // ── Logs ─────────────────────────────────
  void _addLog(String action, String prenom, String details,
               int uid, String un, String role) {
    final l = AppLog(id: newId(), userId: uid, date: DateTime.now(),
      user: un, prenom: prenom, role: role, action: action, details: details);
    DB.addLog(l);
    logs.insert(0, l);
    if (logs.length > 500) logs = logs.sublist(0, 500);
    notifyListeners();
  }
  ...
  void logAct(String action, String details) {
    final u = currentUser;
    if (u == null) return;
    _addLog(action, u.prenom, details, u.id, u.username, u.role);
  }
...
class _LoginState extends State<LoginScreen> {
  ...
  Future<void> _doLogin(AppState st) async {
    ...
    if (!ok) setState(() => _error = 'Identifiants incorrects ou compte inactif.');
  }
}
...
class BackupPage extends StatelessWidget {
  ...
  Future<void> _exportData(BuildContext ctx, AppState st) async {
    final data = {
      ...
      'version': '2.0',
      'settings': st.settings.toMap(),
      'categories': st.categories.map((c) => c.toMap()).toList(),
      'products': st.products.map((p) => p.toMap()).toList(),
      'users': st.users.map((u) => {...u.toMap(), 'password': '***'}).toList(),
    ...
    st.logAct('EXPORT', 'Export JSON des données');
  }
